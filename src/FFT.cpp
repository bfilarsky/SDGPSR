...
FFT::FFT(unsigned size) {
	fftwVector in(size);
	fftwVector out(size);
	forward_   = fftw_plan_dft_1d(size, reinterpret_cast<fftw_complex*>(&in[0]), reinterpret_cast<fftw_complex*>(&out[0]), FFTW_FORWARD,  FFTW_MEASURE);
	reverse_   = fftw_plan_dft_1d(size, reinterpret_cast<fftw_complex*>(&in[0]), reinterpret_cast<fftw_complex*>(&out[0]), FFTW_BACKWARD, FFTW_MEASURE);
	forwardIp_ = fftw_plan_dft_1d(size, reinterpret_cast<fftw_complex*>(&in[0]), reinterpret_cast<fftw_complex*>(&in[0]),  FFTW_FORWARD,  FFTW_MEASURE);
	reverseIp_ = fftw_plan_dft_1d(size, reinterpret_cast<fftw_complex*>(&in[0]), reinterpret_cast<fftw_complex*>(&in[0]),  FFTW_BACKWARD, FFTW_MEASURE);
}
