...
double OrbitalData::currentNavWordTimeOfWeek(void){
	...
	if (timeFromEpochClock < -302400.0)
		timeFromEpochClock += 604800.0;
	double clockCorrection = clockData_.af0_ + clockData_.af1_ * timeFromEpochClock + clockData_.af2_ * timeFromEpochClock * timeFromEpochClock;
	...
	double deltaTr = F_RELATIVISTIC * currentEphemeris_.ecc_ * currentEphemeris_.semiMajorAxisSqrt_ * sin(eccentricAnomaly);
...
Vector3d OrbitalData::satellitePosition(double timeOfWeek){
	...
	double corrRightAscension = currentEphemeris_.rightAscension_ + (currentEphemeris_.rateOfRightAscen_ - OMEGA_EARTH) * timeFromEpoch
